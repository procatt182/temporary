...
export default function DashboardLayout({
  ...
  const { user, loading, userNotProvisioned, signOut } = useAuth();
  ...
  // User not provisioned gate
  if (userNotProvisioned) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-4 grid-bg">
        <div className="pointer-events-none fixed inset-0 overflow-hidden">
          <div className="absolute -left-40 -top-40 h-[500px] w-[500px] rounded-full bg-primary/5 blur-[100px]" />
        ...
        <div className="relative z-10 w-full max-w-md animate-fade-in-up">
          <div className="glass-glow rounded-2xl p-8 text-center">
            ...
            <h1 className="mb-2 text-2xl font-semibold text-foreground">Account Not Found</h1>
            <p className="mb-6 text-sm text-muted-foreground leading-relaxed">
              Your account has not been created yet. Please contact an admin or join our Discord to get started.
            </p>
            <div className="flex flex-col gap-3">
              <Button
                asChild
